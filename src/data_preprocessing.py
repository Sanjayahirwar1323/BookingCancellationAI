...
class DataProcessor:
    ...
    def __init__(self, train_path, test_path, processed_dir, config_path): ## where to store processed data ,
        self.train_path = train_path
        self.test_path = test_path
        self.processed_dir = processed_dir

        self.config = read_yaml(config_path)  ## read config.yaml
    ...
    def preprocess_data(self,df):
        try:
            ...
            cat_cols = self.config["data_processing"]["categorical_columns"] ## under config.yaml cat col
            num_cols = self.config["data_processing"]["numerical_columns"]

            logger.info("Applying Label Encoding")

            label_encoder = LabelEncoder() ##categorical values (textual data) into numerical values.
            ...
            skewness = df[num_cols].apply(lambda x:x.skew()) ## only for num colm as cat do not have skewness
    ...
    def process(self): ## combining all steps
        try:
